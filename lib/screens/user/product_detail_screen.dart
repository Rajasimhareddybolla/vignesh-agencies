...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  void _initDefaultSelection() {
    if (_product != null && _product!.variations.isNotEmpty) {
      ...
      _selectedAttributes = Map.from(_selectedVariation!.attributes);
    }
  }
  ...
  double? get _originalPrice {
     if (_selectedVariation != null) {
       ...
       return _selectedVariation!.offerPrice != null ? _selectedVariation!.price : null;
     }
              ...
              child: Column(
                ...
                children: [
                   ...
                   if (_product!.variations.isNotEmpty) ...[
                     ...
                     Wrap(
                       spacing: 8,
                       children: _product!.variations.map((v) {
                         ...
                         return ChoiceChip(
                           ...
                           selected: isSelected,
                           onSelected: (selected) {
                             if (selected) setState(() => _selectedVariation = v);
                           },
                   ...
                   Text(
                     ...
                     style: const TextStyle(color: AppTheme.textSecondaryLight, height: 1.5),
                   ),
                       ...
                       child: Column(
                         children: _product!.specifications.entries.map((e) {
                           return Container(
                             ...
                             decoration: BoxDecoration(
                               border: Border(bottom: BorderSide(color: AppTheme.borderLight.withAlpha(100))),
                             ),
                             child: Row(
                               crossAxisAlignment: CrossAxisAlignment.start,
                               children: [
                                 Expanded(flex: 2, child: Text(e.key, style: const TextStyle(color: AppTheme.textSecondaryLight))),
                   ...
                   // Highlights
                    if (_product!.highlights.isNotEmpty) ...[
                     ...
                     const SizedBox(height: 8),
                     ..._product!.highlights.map((h) => Padding(
                       padding: const EdgeInsets.only(bottom: 6),
                       child: Row(
                         children: [
        ...
        background: Stack(
          children: [
            CarouselSlider(
              ...
              items: _product!.images.isNotEmpty 
                ? _product!.images.map((url) => CachedNetworkImage(
                    imageUrl: url,
                    fit: BoxFit.cover,
                    width: double.infinity,
                  )).toList()
                : [Container(color: AppTheme.backgroundLight, child: const Center(child: Icon(Icons.image, size: 50)))],
            ),
             Positioned(
              bottom: 20,
              left: 0,
